...
class WhiteDwarf:
    ...
    # Plotting
    def plot_profile(self, type, ax=None, xscale=None, yscale=None, title=None):
        if ax is None:
            fig, ax = plt.subplots()

        if type == 'rho':
            ax.plot(self.rbar2r(self.R_profile), self.rhobar2rho(self.rho_profile))
            ax.set_xlabel('Radius (km)')
            ax.set_ylabel(r'Density (g/cm$^3$)')
        elif type == 'M':
            ax.plot(self.rbar2r(self.R_profile), self.mbar2m(self.M_profile))
            ax.set_xlabel('Radius (km)')
